...
const { core, GITHUB_WORKSPACE, OWNER, REPO, SHA, failOnWarning, ruleContains, unusedDirectiveMessagePrefix } =
  constants;
...
export default function getAnalyzedReport(files: ESLintReport): AnalyzedESLintReport {
  ...
  for (const file of files) {
    ...
    for (const lintMessage of messages) {
      ...
      // Skip if no rules are substrings of the ruleId
      if (ruleContains.length && ruleId && !ruleContains.some((rule) => ruleId.includes(rule))) {
        continue;
      }
